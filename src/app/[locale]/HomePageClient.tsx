...
export default function HomePageClient({
  ...
  const siteUrl =
    (process.env.NEXT_PUBLIC_SITE_URL || "https://www.toonsuniverse.wiki").replace(/\/$/, "");
  ...
  const structuredData = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        genre: ["Survival Horror", "Adventure", "Multiplayer"],
        numberOfPlayers: {
          minValue: 1,
          maxValue: 8,
        },
        offers: {
          ...
          url: homeLinks.roblox,
        },
      },
      {
        "@type": "VideoObject",
        name: homeVideo.title,
        description:
          "Official Toons Universe trailer featuring INNOVA gameplay and survival mechanics.",
        uploadDate: "2026-05-01",
        thumbnailUrl: `${siteUrl}/images/hero.webp`,
        embedUrl: `https://www.youtube.com/embed/${homeVideo.id}`,
        url: homeVideo.url,
      },
    ],
  };
    ...
    <div className="home-shell min-h-screen bg-background text-foreground">
            ...
            {t.modules.lucidBlocksCreaturesAndEnemies.creatures.map(
              (c: any, index: number) => (
                ...
                >
                  <div className="mb-3">
                    <span
                      className={`text-xs px-2 py-1 rounded-full border ${["Hostile Enemy", "Major Threat", "Elite Threat"].includes(c.role) ? "bg-[hsl(var(--nav-theme)/0.18)] border-[hsl(var(--nav-theme)/0.45)] text-[hsl(var(--nav-theme-light))]" : "bg-[hsl(var(--nav-theme)/0.1)] border-[hsl(var(--nav-theme)/0.3)]"}`}
                    >
                      {c.role}
                    </span>
                  </div>
      ...
      <Suspense fallback={<LoadingPlaceholder />}>
        <CTASection
          ...
          communityUrl={homeLinks.discord}
          gameUrl={homeLinks.roblox}
        />
      </Suspense>
