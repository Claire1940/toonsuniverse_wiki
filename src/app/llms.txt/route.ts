...
export async function GET() {
  ...
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Toons Universe Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} is a comprehensive Roblox wiki providing codes, agents, floors, resources, and community guides.`)
  lines.push('')
